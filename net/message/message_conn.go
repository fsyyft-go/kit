...
type (
	// Conn 自定义消息包传输时使用的网络连接。
	//
	// 多个 goroutines 可以同时调用 Conn 上的方法。
	Conn interface {
		// Close 关闭连接。
		//
		// 任何阻塞的读或写操作将被解除阻塞并返回错误。
		Close() error
		// LocalAddr 返回本地网络地址。
		LocalAddr() net.Addr
		// RemoteAddr 返回远程网络地址。
		RemoteAddr() net.Addr
		// SetDeadline 设置读写相关的截止时间，等价于同时调用 SetReadDeadline 和 SetWriteDeadline 。
	...
	conn struct {
		...
		closed       bool
		closedLocker sync.Locker

		messageRead  chan Message
		messageWrite chan Message
...
// Read 从连接中读取数据；
// 如果读取时间超过 SetReadDeadline 和 SetDeadline 的时间，将返回一个错误。
func (c *conn) Read(b []byte) (n int, err error) {
	return c.conn.Read(b)
}

// Write 向连接中写数据；
// 如果写的时间超过 SetWriteDeadline 和 SetDeadline 的时间，将返回一个错误。
...
func (c *conn) receive(ctx context.Context) {
	...
	if c.heartbeatInterval > 0 {
		// 如果有发送心跳，则默认超时时间为发送心跳的 2 倍时长；心跳是双向的，两倍足够了。
...
func WrapConn(c net.Conn, heartbeatInterval time.Duration) *conn {
	newConn := &conn{
		...
		messageRead:       make(chan Message, 5120),
		messageWrite:      make(chan Message, 5120),
